...
#   define LOG_REFS(x, ...) aocl::Log::print(aocl::Log::WARNING, x, ##__VA_ARGS__)
#endif
...
bool aocl::OclLib::init(const char *fileName)
{
    if (!m_initialized) {

        m_loader      = fileName == nullptr ? defaultLoader() : String(fileName);
...
const char *aocl::OclLib::lastError()
{
    return uv_dlerror(&oclLib);
}


void aocl::OclLib::close()
{
    uv_dlclose(&oclLib);
}


bool aocl::OclLib::load()
{
    try {
...
aocl::String aocl::OclLib::defaultLoader()
{
...
#   elif defined(_WIN32)
    return "OpenCL.dll";
#   else
    return "libOpenCL_system.so";
#   endif
}


cl_command_queue aocl::OclLib::createCommandQueue(cl_context context, cl_device_id device, cl_int *errcode_ret) noexcept
{
...
cl_command_queue aocl::OclLib::createCommandQueue(cl_context context, cl_device_id device)
{
    cl_int ret = 0;
...
cl_context aocl::OclLib::createContext(const cl_context_properties *properties, cl_uint num_devices, const cl_device_id *devices, void (CL_CALLBACK *pfn_notify)(const char *, const void *, size_t, void *), void *user_data, cl_int *errcode_ret)
{
...
cl_context aocl::OclLib::createContext(const std::vector<cl_device_id> &ids)
{
    cl_int ret = 0;
...
cl_int aocl::OclLib::buildProgram(cl_program program, cl_uint num_devices, const cl_device_id *device_list, const char *options, void (CL_CALLBACK *pfn_notify)(cl_program program, void *user_data), void *user_data) noexcept
{
...
cl_int aocl::OclLib::enqueueNDRangeKernel(cl_command_queue command_queue, cl_kernel kernel, cl_uint work_dim, const size_t *global_work_offset, const size_t *global_work_size, const size_t *local_work_size, cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) noexcept
{
...
cl_int aocl::OclLib::enqueueReadBuffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr, cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) noexcept
{
...
cl_int aocl::OclLib::enqueueWriteBuffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr, cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) noexcept
{
...
cl_int aocl::OclLib::finish(cl_command_queue command_queue) noexcept
{
...
cl_int aocl::OclLib::getCommandQueueInfo(cl_command_queue command_queue, cl_command_queue_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
    return pGetCommandQueueInfo(command_queue, param_name, param_value_size, param_value, param_value_size_ret);
}


cl_int aocl::OclLib::getContextInfo(cl_context context, cl_context_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
    return pGetContextInfo(context, param_name, param_value_size, param_value, param_value_size_ret);
}


cl_int aocl::OclLib::getDeviceIDs(cl_platform_id platform, cl_device_type device_type, cl_uint num_entries, cl_device_id *devices, cl_uint *num_devices) noexcept
{
...
cl_int aocl::OclLib::getDeviceInfo(cl_device_id device, cl_device_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
...
cl_int aocl::OclLib::getKernelInfo(cl_kernel kernel, cl_kernel_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
    return pGetKernelInfo(kernel, param_name, param_value_size, param_value, param_value_size_ret);
}


cl_int aocl::OclLib::getMemObjectInfo(cl_mem memobj, cl_mem_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
    return pGetMemObjectInfo(memobj, param_name, param_value_size, param_value, param_value_size_ret);
}


cl_int aocl::OclLib::getPlatformIDs(cl_uint num_entries, cl_platform_id *platforms, cl_uint *num_platforms)
{
...
cl_int aocl::OclLib::getPlatformInfo(cl_platform_id platform, cl_platform_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
...
cl_int aocl::OclLib::getProgramBuildInfo(cl_program program, cl_device_id device, cl_program_build_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) noexcept
{
...
cl_int aocl::OclLib::getProgramInfo(cl_program program, cl_program_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret)
{
...
cl_int aocl::OclLib::release(cl_command_queue command_queue) noexcept
{
...
cl_int aocl::OclLib::release(cl_context context) noexcept
{
...
cl_int aocl::OclLib::release(cl_device_id id) noexcept
{
...
cl_int aocl::OclLib::release(cl_kernel kernel) noexcept
{
...
cl_int aocl::OclLib::release(cl_mem mem_obj) noexcept
{
...
cl_int aocl::OclLib::release(cl_program program) noexcept
{
...
cl_int aocl::OclLib::setKernelArg(cl_kernel kernel, cl_uint arg_index, size_t arg_size, const void *arg_value) noexcept
{
...
cl_int aocl::OclLib::unloadPlatformCompiler(cl_platform_id platform) noexcept
{
    return pUnloadPlatformCompiler(platform);
}


cl_kernel aocl::OclLib::createKernel(cl_program program, const char *kernel_name, cl_int *errcode_ret) noexcept
{
...
cl_kernel aocl::OclLib::createKernel(cl_program program, const char *kernel_name)
{
    cl_int ret = 0;
...
cl_mem aocl::OclLib::createBuffer(cl_context context, cl_mem_flags flags, size_t size, void *host_ptr)
{
    cl_int ret = 0;
...
cl_mem aocl::OclLib::createBuffer(cl_context context, cl_mem_flags flags, size_t size, void *host_ptr, cl_int *errcode_ret) noexcept
{
...
cl_mem aocl::OclLib::createSubBuffer(cl_mem buffer, cl_mem_flags flags, size_t offset, size_t size, cl_int *errcode_ret) noexcept
{
...
cl_mem aocl::OclLib::createSubBuffer(cl_mem buffer, cl_mem_flags flags, size_t offset, size_t size)
{
    cl_int ret = 0;
...
cl_mem aocl::OclLib::retain(cl_mem memobj) noexcept
{
...
cl_program aocl::OclLib::createProgramWithBinary(cl_context context, cl_uint num_devices, const cl_device_id *device_list, const size_t *lengths, const unsigned char **binaries, cl_int *binary_status, cl_int *errcode_ret) noexcept
{
...
cl_program aocl::OclLib::createProgramWithSource(cl_context context, cl_uint count, const char **strings, const size_t *lengths, cl_int *errcode_ret) noexcept
{
...
cl_program aocl::OclLib::retain(cl_program program) noexcept
{
...
cl_uint aocl::OclLib::getNumPlatforms() noexcept
{
    cl_uint count   = 0;
    cl_int ret      = 0;
...
cl_uint aocl::OclLib::getUint(cl_command_queue command_queue, cl_command_queue_info param_name, cl_uint defaultValue) noexcept
{
...
cl_uint aocl::OclLib::getUint(cl_context context, cl_context_info param_name, cl_uint defaultValue) noexcept
{
...
cl_uint aocl::OclLib::getUint(cl_device_id id, cl_device_info param, cl_uint defaultValue) noexcept
{
...
cl_uint aocl::OclLib::getUint(cl_kernel kernel, cl_kernel_info  param_name, cl_uint defaultValue) noexcept
{
...
cl_uint aocl::OclLib::getUint(cl_mem memobj, cl_mem_info param_name, cl_uint defaultValue) noexcept
{
...
cl_uint aocl::OclLib::getUint(cl_program program, cl_program_info param, cl_uint defaultValue) noexcept
{
...
cl_ulong aocl::OclLib::getUlong(cl_device_id id, cl_device_info param, cl_ulong defaultValue) noexcept
{
...
cl_ulong aocl::OclLib::getUlong(cl_mem memobj, cl_mem_info param_name, cl_ulong defaultValue) noexcept
{
...
std::vector<cl_platform_id> aocl::OclLib::getPlatformIDs() noexcept
{
...
aocl::String aocl::OclLib::getProgramBuildLog(cl_program program, cl_device_id device) noexcept
{
    size_t size = 0;
...
aocl::String aocl::OclLib::getString(cl_device_id id, cl_device_info param) noexcept
{
    return getOclString(OclLib::getDeviceInfo, id, param);
}


aocl::String aocl::OclLib::getString(cl_kernel kernel, cl_kernel_info param_name) noexcept
{
    return getOclString(OclLib::getKernelInfo, kernel, param_name);
}


aocl::String aocl::OclLib::getString(cl_platform_id platform, cl_platform_info param_name) noexcept
{
    return getOclString(OclLib::getPlatformInfo, platform, param_name);
}


aocl::String aocl::OclLib::getString(cl_program program, cl_program_info param_name) noexcept
{
